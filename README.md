...
c_rehash is a perl script necessary for the use of openssl and is supposed to
be generated by the openssl build. Recently I installed openssl using the
usual apt tools on a laptop running Kali Linux in order to build my Codex
project. c_rehash was not installed by either the openssl package nor the perl
package. I had to "borrow" it from another Linux system. This script is
copyrighted by the OpenSSL project and is licensed under the Apache license.
